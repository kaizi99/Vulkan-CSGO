...
bsp_rendering_data bsp_rendering_prepare(bsp_parsed* bsp) {
    ...
    renderingData.clusterRenderingData.resize(bsp->clusterCount);
    for (int i = 0; i < bsp->clusterCount; i++) {
        cluster* c = bsp->clusters + i;
        ...
        renderingData.clusterRenderingData[i]
    }
}